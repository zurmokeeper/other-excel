...
export function parseXF(blob: CustomCFB$Blob, length: number) {
    ...
    return { fontIndex: ifnt, ifmt, fLocked, fHidden, fStyle, f123Prefix, ixfParent, data: data};
}
...
function parseCellStyleXF(blob: CustomCFB$Blob, length: number, style: number, options?: any) {
    ...
    const alignment = {horizontal, vertical, wrapText, textRotation, indent: cIndent, shrinkToFit, readingOrder};
